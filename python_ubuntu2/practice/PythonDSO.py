...
class LecroyBinFormat:
	def __init__(self,wfdata):
		...
		self.DESCRIPTOR_NAME=struct.unpack(endian+'16s',wfdata[0:16])[0].decode('UTF-8')
		self.TEMPLATE_NAME=struct.unpack(endian+'16s',wfdata[16:16+16])[0].decode('UTF-8')
		...
		self.INSTRUMENT_NAME=struct.unpack(endian+'16s',wfdata[76:76+16])[0].decode('UTF-8')
		self.INSTRUMENT_NUMBER=struct.unpack(endian+'I',wfdata[92:92+4])[0]
		self.TRACE_LABEL=struct.unpack(endian+'16s',wfdata[96:96+16])[0].decode('UTF-8')
		...
		self.NOM_SUBARRAYT_COUNT=struct.unpack(endian+'H',wfdata[174:174+2])[0]
		...
		if self.COMM_TYPE==0:
			self.ad_value=struct.unpack(endian+str(self.WAVE_ARRAY_COUNT)+'b',wfdata[self.WAVE_DESCRIPTOR:self.WAVE_DESCRIPTOR+self.WAVE_ARRAY_COUNT])
		else:
			self.ad_value=struct.unpack(endian+str(self.WAVE_ARRAY_COUNT)+'h',wfdata[self.WAVE_DESCRIPTOR:self.WAVE_DESCRIPTOR+self.WAVE_ARRAY_COUNT*2])
	...
	def scaled_waveform(self):
		...
		for i in range(self.WAVE_ARRAY_COUNT):
			scaled_wf.append(self.ad_value[i]*self.VERTICAL_GAIN-self.VERTICAL_OFFSET)
		return scaled_wf
...
class LecroyVICP:
	...
	def writestring(self,cmdstr):
		cmdstr+='\n'
		cmdlen=len(cmdstr)
		sbuff=struct.pack(">Bbbbi"+str(cmdlen)+"s",129,1,0,0,cmdlen,cmdstr.encode("UTF-8"))
